...
/*function $(selector,obj){
	...
	if(typeof selector=="string"){//判断selector是否是字符串
		...
		}else if(/^[a-z][a-z|1-6]{0,9}$/g.test(selector)){
...
function $(selector,obj){
  var obj=obj||document;
    if(typeof selector=="string"){
      ...
      }else if(/^[a-z|1-10]{1,10}$/g.test(selector)){//返回的是个集合
...
*/
function getChilds(father,text){
      var text=text||false;
      var childs=father.childNodes;
      var arr=[];
...
//获取第一个子节点
 ...
 function getFirst(father,text){
            return getChilds(father,text)[0];
    }
 /********************************************/
 //获取最后一个节点
 ...
 function getLast(father,text){
      var index=getChilds(father,text).length-1;
            return getChilds(father,text)[index];
    }
 /*********************************************/
 //获取指定的子节点
 ...
 function getNum(father,text,num){
      return getChilds(father,text)[num];
    }
/*********************************************/
 //获取下一个兄弟节点
    ...
    function getnextSibling(obj){
      ...
      if(next==null){//防止最后一个没有下一个子元素的出现(且没有换行)
        return false;
      }
      
        while(next.nodeType==8||(next.nodeType==3&&next.nodeValue.replace(/^\s*|\s*$/g,"")=="")){
                  next=next.nextSibling;
                  if(next==null){//防止最后一个没有下一个子元素的出现(且有换行)
                    return false;
                  }
...
//获取上一个兄弟节点
    ...
    function getlastSibling(obj){
      ...
      if(last==null){//防止最后一个没有下一个子元素的出现(且没有换行)
        return false;
      }
      
        while(last.nodeType==8||(last.nodeType==3&&last.nodeValue.replace(/^\s*|\s*$/g,"")=="")){
                  last=last.previousSibling;
                  if(last==null){//防止最后一个没有下一个子元素的出现(且有换行)
                    return false;
                  }
...
//把obj1放到obj2的后面
    /*function insertafter(obj1,obj2){
      var obj2Next=getDown(obj2);
      insertbefore(obj1,obj2Next);
    }*/
    function insertafter(obj1,obj2){
      var obj2Next=getnextSibling(obj2);
      var parent=obj2.parentNode;
      if(obj2Next==false){
        parent.appendChild(obj1);
      }else{
        parent.insertBefore(obj1,obj2Next);
      }
